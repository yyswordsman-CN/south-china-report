...
> 技能标识 `south-china-report` 为稳定兼容 ID；用户可见名称与触发描述按通用经营报告定位表达。当前内容版本 V2.12.0，完整历史见 `CHANGELOG.md`。
>
...
4. Read 对应模板 (**CSS 唯一真相源**) → 步骤 6；模板出厂即紧凑，标准档才需 `sed` 一行退回 → 步骤 7
...
- **期间是必填口径**：`map.caliber.period` 支持 `2025-12`、上/中/下旬、`2025Q4`、`2025H1/H2`、全年、`start..end` 或 `{start,end}`。未明确填写时默认 BLOCKED；只有用户明确允许推断时才可设 `allow_inferred_period: true`。总量、YoY、趋势、维度、产品、集中度和目标共用同一 `period_lock`。
- **目标口径不猜**：目标列存在时必须显式确认 `target_measure=amount|qty`，禁止金额与销量目标错配；`target_aggregation` 可用 `sum|first_per_group|unique_per_period`，默认 `auto` 只接受锁定期间唯一一个目标值，出现多行即局部 BLOCKED，需结合 `target_grain/target_frequency` 确认。输出 `actual/plan/achievement_rate/gap`。
- **产品不再只识别**：`map.roles.product` 会输出产品排名、增长/下滑双榜和贡献度，均受 `period_lock` 约束。
- **数据质量诚实化**：无 ID 也查完全重复行；时间/金额/数量/目标解析阈值与维度 Jaccard/客户 Top5 阈值可配；出现负数净额分项时停算份额/HHI；PVM caveat 会阻断强量价结论。
- **统计洞察层**：`stat-insights.py` 输出 MK 趋势、稳健 Z、断崖/引擎/结构位移/增速贡献、HHI、量价象限与问题清单；连续负增长必须日历相邻，阈值为可调启发式而非普适定律。
...
> **风格档位 (V2.12.0)**: 三份模板的 `<html>` 均已烘焙 `data-density="compact"`，复制后就是默认紧凑成品。叙事标准风是显式退回档，仅在用户明确要求时移除该属性。**交付物不含任何页面内切换控件**。`validate-report.mjs` 的「密度轴」项会回报实际档位，据此确认默认或显式退回是否生效。
>
...
2. 填写模板内 `#south-china-report-meta` JSON 契约：`requested_period`、`source.path`、`source.sha256`、`report_mode`、`key_metrics`（可选 `metrics_sha256`）必须与本次请求、数据源与 metrics.json 一致；文件源的安全标签和指纹可直接取 `metrics.meta.source_path/source_sha256`（不会暴露本机绝对路径），任何 `[...]` 占位值都会阻断成品交付。SQL 的指纹仅覆盖查询文本，必须同时披露 `source_fingerprint_scope/source_caveat`，不得冒充数据快照。
3. 页面所有可见业务数字必须使用 `data-metric="metrics.json点分路径"`；章节号、日期、页码等非业务数字必须用 `data-number-exempt="具体原因"` 显式豁免，不允许默默漏过。
4. 页面存在 ECharts 时必须填写唯一、默认 `version: 2` 的 `#south-china-report-runtime-contract`：标量系列可用等长 `metrics` 简写；坐标、递归树和 custom 嵌套结构必须用 JSON Pointer `bindings/exemptions` 覆盖每个数值/null 叶子。透明基座等纯辅助系列才可整系列 `exempt`。V1 只作标量兼容，完整规则见 `references/runtime-metrics-contract.md`。
5. 按 §3 布局结构填充 Chapter，每个 Chapter 遵循 §6 叙事规则。
6. **选图必须走引擎**：每个 Chapter 先过 `chart-selection-guide.md` §0 意图路由表，紧凑档优先取 §7 密集形态；再从 `references/chart-patterns.md` 取代码 + `scripts/echarts-corporate-themes.js` 主题。
7. 图表遵守 `chart-selection-guide.md` §6 IBCS 语义：时间=横轴/分类=水平条形、实际实心/计划空心/预测斜纹、三段式口径副标题、红绿只表差异。
8. 组件使用 `references/component-patterns.md` 中的代码片段。
...
- **P0 阻断**: Emoji / 数字排版覆盖 / 图表容器高度 / 占位符与待校验状态 / 未确认审计 PASS；严格离线模式另外阻断所有外部与相对资源
...
| `references/checklist.md`                  | P0/P1 快速核对清单（当前 14 项 P0 + 6 项 P1）                                  | 生成后快速自检                 |
...
| `scripts/prep-source.py`                   | 多源数据画像、锁定请求期间、目标聚合与清洗校验 → `metrics.json`                | **有数据源时第一步** (步骤4) |
| `scripts/stat-insights.py`                 | 统计洞察层：趋势显著性/异常月/断崖引擎/贡献分解/HHI/量价象限                    | **build 之后第二步**；问题发现章证据源 |
...
| `scripts/verify-numbers.mjs`               | 可见业务数字覆盖率与 `data-metric` 真值一致性 Gate（默认要求 100%）             | 有 `metrics.json` 时交付前必跑 |
...
9. 生成后跑齐步骤 9 的静态/离线/运行时/截图 Gate → 读 `references/checklist.md` 快速自检 → 读 `references/mckinsey-quality-gate.md` 最终检查
...
> 当前版本 V2.12.0。历史变更、缺陷复现和回归证据统一维护在 `CHANGELOG.md`，不在运行时指令中重复。
