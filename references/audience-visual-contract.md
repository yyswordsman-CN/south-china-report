...
- **Data Contract 必答项 (V2.5)**：① 数据截止日期与同比基线口径？② **有无目标/预算/计划字段？无 → 达成率/子弹图/进度条/完成度全部降级为 YoY/结构/趋势 (见 `chart-selection-guide.md` §0.1b)，禁止编造目标**。③ 关键字段有无跨年编码变更 (拿来做同比前先验一致性，如"产品定位"重分类会造成假涨跌)？④ 有无字段信号过弱 (如国补占比 <2%)，弱信号不单独成章。
